...
void sort(int *array, size_t length)
{
    ...
    {
        indexs[i] = (int *)malloc(2 * sizeof(int));        
    }

    // tab qui contient les longueurs de ces tableaux
    int *lgs = malloc(length * sizeof(int));
    // iterateur et nbr de sous tableaux découpés et réarrangés
    size_t i = 0, nbr_tab = 0;

    //découpage en sous tableaux et mise en place de la pile + tailles
    while(i < length - 1)
    {
        indexs[nbr_tab][0] = i;
        i = findRun(array, i, length - 1, 100);
        indexs[nbr_tab][1] = i;

        lgs[nbr_tab] = (indexs[nbr_tab][1] - indexs[nbr_tab][0]) + 1;
